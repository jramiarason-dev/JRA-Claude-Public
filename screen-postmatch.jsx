...
const PostMatchScreen = ({ lang, matchId, setRoute, sport }) => {
  ...
  // Use ANALYSES data for performers if available
  const analysisData = window.ANALYSES?.[matchId];
  const _fallbackPerformers = sportType === 'football' ? [
  ...
  const performers = analysisData ? [
    ...(analysisData.joueurs?.home || []).slice(0, 2).map((p, i) => ({
      num: i + 1, name: p.nom, team: match.home.code, pos: p.poste,
      rating: p.note, stat: p.stats,
    })),
    ...(analysisData.joueurs?.away || []).slice(0, 2).map((p, i) => ({
      num: i + 1, name: p.nom, team: match.away.code, pos: p.poste,
      rating: p.note, stat: p.stats,
    })),
  ] : _fallbackPerformers;
    ...
    <div className="page">
        ...
        <div className="match-teams" style={{ gap: 24 }}>
          <div className="team" style={{ gap: 16 }}>
            ...
            <div>
              ...
              {match.home.rank != null && <div style={{fontSize: 12, color: '#888'}}>#{match.home.rank}</div>}
            </div>
          </div>
          ...
          <div className="team away" style={{ gap: 16 }}>
            ...
            <div style={{textAlign: 'right'}}>
              ...
              {match.away.rank != null && <div style={{fontSize: 12, color: '#888'}}>#{match.away.rank}</div>}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="verdict fade-in" style={{ marginBottom: 20 }}>
        <p className="verdict-quote">
          {analysisData?.verdict?.home_txt || (lang === 'fr' ? match.verdict_fr : match.verdict_en)}
        </p>
